...
Hold the whole argument in one hand. The cooldown takes credit for a takedown it did not perform. The catalog records what is convenient and omits what is killing you. The scanner guesses at a tree the package manager already built, then bills you for the guess. Three different failures, one shape: a control standing where it can take the credit, not where it can do the work. Pull the detection out from under each, and there is nothing left to defend.

So put detection where it actually changes the outcome. At the source, at the moment of publication, in front of the attacker. Everything else in this book follows from that one move. Scan before you publish, and the cooldown has nothing left to do. Track the malware as a first-class object, and the catalog stops lying by omission. Free the data, move it on an exchange instead of a catalog, and feed it to the things doing the installing, so the agent at the keyboard reasons from a VDR and a VEX that were already moving before the advisory shipped. Trust the package manager that did the install, gate the unsafe thing behind `--install-unsafe`, treat a hash mismatch as the malware it almost always is, and the truth lives where the work happened. None of these are separate programs. They are the same sentence read from different ends.
